...
Khám phá dữ liệu & Tiền xử lý (EDA & Pre-processing)  
...
Ví dụ mã Python với scikit-learn:  from sklearn.model_selection import train_test_split
...
Ví dụ Grid Search:  from sklearn.model_selection import GridSearchCV
...
Đánh giá & Triển khai (Evaluation & Deployment)  
...
Đọc CSV:  import pandas as pd
df = pd.read_csv("data.csv")


Chuyển đổi dữ liệu SQL query thành DataFrame:  import sqlite3
...
Kết nối sqlite3:  import sqlite3
...
Kết nối MongoDB với Python (pymongo):  from pymongo import MongoClient
...
Ví dụ với Dask:  import dask.dataframe as dd
...
Log Transformation: Dùng cho dữ liệu lệch (skewed), ví dụ: giá nhà.  import numpy as np
...
Ví dụ SMOTE:  from imblearn.over_sampling import SMOTE
...
Text Data: Chuyển văn bản thành số (TF-IDF, Word2Vec).  from sklearn.feature_extraction.text import TfidfVectorizer
vectorizer = TfidfVectorizer()
X_text = vectorizer.fit_transform(df["text_column"])


Datetime: Trích xuất đặc trưng (ngày, tháng, giờ).  df['date'] = pd.to_datetime(df['date'])
...
Ví dụ t-test:  from scipy.stats import ttest_ind
...
VIF (Variance Inflation Factor): VIF > 5 thì loại bỏ biến.  from statsmodels.stats.outliers_influence import variance_inflation_factor
...
Chọn tham số tối ưu, tránh overfitting.  from sklearn.linear_model import RidgeCV
...
SVM: Tìm hyperplane phân cách tối ưu.  from sklearn.linear_model import LogisticRegression
...
PCA: Giảm chiều dữ liệu, giữ thông tin chính.  from sklearn.cluster import KMeans
...
Cấu trúc: Input, hidden layers, output với activation (ReLU, sigmoid).  from keras.models import Sequential
...
Đường dẫn tương đối: Nếu file Markdown ở root repo, dùng:![Tên hình](images/filename.png)
...
Mở ảnh trên GitHub (ví dụ: https://github.com/SarenFan/First-week-AIL303m/blob/main/images/ml_workflow.png).
Click nút Raw để lấy URL: https://raw.githubusercontent.com/SarenFan/First-week-AIL303m/main/images/ml_workflow.png.
Dùng trong Markdown:![Tên hình](https://raw.githubusercontent.com/SarenFan/First-week-AIL303m/main/images/filename.png)
...
Nếu click vào link raw (như https://raw.githubusercontent.com/SarenFan/First-week-AIL303m/main/images/ml_workflow.png) và nhận lỗi 404 Not Found, hãy làm như sau:

Kiểm tra thư mục images/: 
Mở repo trên GitHub (https://github.com/SarenFan/First-week-AIL303m), đảm bảo thư mục images/ tồn tại và chứa file ảnh đúng tên (ví dụ: ml_workflow.png).
Nếu không thấy: Tạo thư mục images/ cục bộ, thêm ảnh, và commit:mkdir images
# Copy ảnh vào images/
git add images/
git commit -m "Add images for Markdown"
git push origin main
...
Kiểm tra branch: Nếu repo dùng master thay vì main, sửa URL thành:![Tên hình](https://raw.githubusercontent.com/SarenFan/First-week-AIL303m/master/images/filename.png)


Tạo ảnh nếu chưa có: Dùng Python để tạo (ví dụ heatmap):import seaborn as sns
...
Cập nhật đường dẫn trong Markdown, ví dụ:![Tên hình](/assets/filename.png)
...
Cung cấp link repo GitHub (ví dụ: https://github.com/SarenFan/First-week-AIL303m).
